      ...
      <div className="bg-white rounded-3xl p-6 max-w-lg w-full shadow-2xl space-y-5">
        <div className="flex items-center justify-between border-b border-slate-100 pb-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-base text-slate-900">My Member Profile</h3>
          </div>
          <button
            onClick={onClose}
            className="p-1 text-slate-400 hover:text-slate-600 rounded-xl transition"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="flex items-center gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-200">
          ...
          <div className="space-y-0.5">
            <h4 className="font-black text-slate-900 text-base">{currentUser.name}</h4>
            ...
            <div className="p-3 bg-amber-50/60 rounded-2xl border border-amber-200/80 space-y-1.5">
              ...
              <div className="flex flex-wrap gap-1.5">
                {linkedMember.ministryTeams.map((team, idx) => (
                  <span key={idx} className="bg-white text-slate-900 text-xs font-bold px-2.5 py-0.5 rounded-xl border border-amber-300 shadow-2xs">
                    {team}
                  </span>
                ))}
              </div>
        ...
        <div className="pt-2 flex justify-end">
          <button
            onClick={onClose}
            className="px-5 py-2 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs rounded-2xl shadow-md transition"
          >
            Close Profile
          </button>
        </div>
