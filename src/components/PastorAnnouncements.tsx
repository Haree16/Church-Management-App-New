  ...
  return (
    <div className="space-y-4">
      ...
      <div 
        ...
        className="dark-hero-panel bg-gradient-to-r from-amber-950 via-slate-900 to-amber-900 text-white rounded-3xl p-5 sm:p-7 shadow-xl relative overflow-hidden border border-amber-800/40"
      >
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            <div 
              ...
              style={{ color: '#fde68a' }}
            >
              <Megaphone className="w-3.5 h-3.5 text-[#fbbf24]" style={{ color: '#fbbf24' }} />
              <span>Pastor's Desk & Official Announcements</span>
            </div>
        ...
        {safeAnnouncements.map((item) => (
          <div
            ...
            className={`p-4 sm:p-6 rounded-3xl border shadow-sm transition space-y-3 ${
              item.isPinned
                ? 'bg-amber-950/30 border-amber-500/50 ring-1 ring-amber-400/20'
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                {item.isPinned && (
                  <span className="inline-flex items-center gap-1 text-[11px] font-extrabold text-amber-300 bg-amber-950/80 border border-amber-800 px-2.5 py-0.5 rounded-full">
                    <Pin className="w-3 h-3 fill-amber-400 text-amber-400" /> Pinned Notice
                  </span>
                )}
                <span className="text-[10px] font-extrabold uppercase tracking-wider text-slate-300 bg-slate-800 border border-slate-700 px-2.5 py-0.5 rounded-full">
                  {item.category}
                </span>
              </div>
            ...
            <div className="pt-3 border-t border-slate-800 flex items-center justify-between text-xs text-slate-400 font-medium">
              <div className="flex items-center gap-2">
                ...
                <span className="text-slate-300">{item.authorName}</span>
              </div>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-2.5 py-1.5 rounded-lg text-emerald-300 bg-emerald-950/80 hover:bg-emerald-900/80 border border-emerald-800 transition flex items-center gap-1.5 text-xs font-bold"
                  title="Share announcement to WhatsApp"
                >
                  {copiedId === item.id ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <MessageSquare className="w-3.5 h-3.5 text-emerald-400" />}
                ...
                {canPublish && (
                  <button
                    ...
                    className="p-1.5 rounded-lg text-slate-400 hover:text-amber-400 hover:bg-slate-800 transition flex items-center gap-1 text-xs font-semibold"
                ...
                {canPublish && onDeleteAnnouncement && (
                  <button
                    ...
                    className="p-1.5 rounded-lg text-slate-400 hover:text-rose-400 hover:bg-rose-950/40 transition flex items-center gap-1 text-xs font-semibold"
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 w-full max-w-md rounded-3xl p-6 shadow-2xl border border-slate-800 space-y-4">
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-sm text-white placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-amber-500"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  >
                    <option value="Sunday Bulletin" className="bg-slate-900 text-white">Sunday Bulletin</option>
                    <option value="General" className="bg-slate-900 text-white">General</option>
                    <option value="Ministry Alert" className="bg-slate-900 text-white">Ministry Alert</option>
      ...
      {announcementToDelete && (
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 w-full max-w-sm rounded-3xl p-6 shadow-2xl border border-slate-800 space-y-4">
            ...
            <div className="text-center space-y-1">
              ...
              <p className="text-xs text-slate-300">
                Are you sure you want to delete <strong className="text-white font-bold">"{announcementToDelete.title}"</strong> ({announcementToDelete.date})?
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 pt-2">
              <button
                ...
                className="py-2.5 px-4 bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 font-bold rounded-xl text-xs transition"
              >
                Cancel
              </button>
              <button
                ...
                className="py-2.5 px-4 bg-rose-600 hover:bg-rose-500 text-white font-bold rounded-xl text-xs shadow-lg shadow-rose-600/30 transition flex items-center justify-center gap-1.5"
              >
                <Trash2 className="w-3.5 h-3.5" />
                Delete
              </button>
